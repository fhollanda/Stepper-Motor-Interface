	...
	def __init__(self, device, debug=False):
		...
		try:
			...
			if str(e).find('No such file or directory') != -1:
				error = "ERROR: Path to USB device does not exist ("+ device + ")"
				print(error)
				...
				error = "Can't open path to device, have you ran: \n"+ "sudo chmod 777 " + device
				print(error)
				raise Exception(error)
				#sys.exit()
...
class tek2024:
	""" The class for the Tektronix TDS2024C oscilloscope
